...
var mapPersonRDD= mapSplitRDD.map(attributes => Person(attributes(0), attributes(1).toInt));

 var df=mapPersonRDD.toDF();
...
df.map(teenager => "Name: " + teenager.getString(0)).show()
