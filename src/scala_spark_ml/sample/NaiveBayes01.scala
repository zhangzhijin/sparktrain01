...
object NaiveBayes01 { 
   ...
   var conf=new SparkConf().setMaster("local[4]").setAppName("NaiveBayes01");
  ...
  .getOrCreate()
   ...
   import  sparkSession.implicits._
      ...
      var file="D:\\zzj\\11_programfile\\eclipse_workspace\\scala_lc\\testfolder\\ml\\NaiveBayes01.txt"
...
val sqlDF = sparkSession.sql("select " +
 ...
 val naiveBayesModel = new NaiveBayes().setLabelCol("affairs").setFeaturesCol("features").fit(training)
   
 //println(lrModel.numClasses)
 // println(lrModel.numFeatures)
   // println(naiveBayesModel.)
   //   println(lrModel.elasticNetParam)
    //    println(lrModel.regParam)
 var predit= naiveBayesModel.transform(test).select($"affairs",$"features", $"rawPrediction",$"probability",$"prediction")
